...
const Comment:React.FC<CommentProps> = ({postID, comment, profilePic, postBy, postComments, commentCreationDate, commentDislikes, commentLikes, postStatus, postDetailsPage, postVotes}) => {

  const currentDate = new Date();
  const postCreationDateObj = new Date(Number(commentCreationDate) * 1000);

  const postCreationDateString = Math.floor((Number(currentDate) - Number(postCreationDateObj)) / (1000 * 60 * 60 * 24));

  return (
    <main className={`${!postDetailsPage && "border-[#D5D5D5] border  p-[15px]"} rounded-[10px] flex flex-col gap-y-[19px]`} key={`post-${postID}`}>
        <section className='flex justify-between items-center'>
          <div className='flex items-center gap-x-3'>
            <Image src={profilePic ? profilePic : userpic} alt='Profile Pic' width={40} height={40} />
        ...
        <div className={`flex ${postDetailsPage ? "flex-col-reverse" : "flex-col"} gap-y-3`}>
          <Link href={postDetailsPage ? `#` : `/app/${postID}`}>
            <section className='flex flex-col gap-y-2'>
              <p className='text-[#2E3035] text-md line-clamp-3'>{comment}</p>
            </section>
          </Link>
        ...
        <section className='flex justify-between items-center'>
          <div className='flex items-center gap-x-3'>
            ...
            <p className='text-[#5B5E65] flex items-center gap-x-2 text-md'><BiSolidLike className='text-xl text-green-400 flex items-center gap-x-2' /><span>{commentLikes} Likes</span></p>

            <p className='text-[#5B5E65] flex items-center gap-x-2 text-md'><BiSolidDislike className='text-xl text-red-400 flex items-center gap-x-2' /><span>{commentDislikes} Dislikes</span></p>
          </div>
