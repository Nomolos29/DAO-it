...
const Post:React.FC<PostProps> = ({id, title, description, profilePic, postBy, postComments, postCreationDate, postDislikes, postLikes, postStatus, postDetailsPage, postStartDate, postEndDate, postImage, postVotes}) => {
  ...
  return (
    <main className={`${!postDetailsPage && "border-[#D5D5D5] border p-3 md:p-[15px]"} rounded-[10px] flex flex-col gap-y-[19px]`} key={`post-${id}`}>
        <section className='flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 sm:gap-0'>
          <div className='flex items-center gap-x-3'>
            {profilePic && <Image src={userpic} alt='Profile Pic' width={40} height={40} className="w-8 h-8 sm:w-10 sm:h-10" />}

            <div className='flex flex-col gap-y-1'>
                <p className='flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-x-2 text-sm sm:text-base'>
                  <span>Proposal by {postBy}</span>
                  <span className='text-[#5B5E65] flex items-center gap-x-2'><Dot />{postCreationDateString}d</span>
                </p>
                <p className='text-[#5B5E65] flex items-center gap-x-1 text-xs sm:text-sm'>
        ...
        <div className={`flex ${postDetailsPage ? "flex-col-reverse" : "flex-col"} gap-y-3`}>
          <Link href={postDetailsPage ? `#` : `/app/${id}`}>
            <section className='flex flex-col gap-y-2'>
              <h3 className='text-base md:text-lg font-medium text-[#232426]'>{title}</h3>
              <p className='text-[#2E3035] text-sm md:text-md line-clamp-3'>{description}</p>
            </section>
          </Link>

          {postImage && <section className='w-full h-[150px] sm:h-[202px]'>
              <Image src={postPic} alt='post image' width={1000} height={1000} className='h-full w-full object-cover rounded-md' />
          </section>}
        </div>
