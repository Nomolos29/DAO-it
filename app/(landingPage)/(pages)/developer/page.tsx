...
const DocumentationPage = () => {
    ...
    return (
        <section className="flex flex-col items-center w-full">
            ...
            <div className="flex flex-col px-4 md:px-10 max-w-screen-2xl md:flex-row w-full gap-x-10 py-10 md:py-20">
                        ...
                        {introduction.map((term, index) => (
                            <Link key={index} href={`#${slugify(term.heading)}`} scroll={false} onClick={() => {setOpen(!open)}}>
                                <div className="flex items-center w-full justify-between gap-x-5">
                                    <span className="text-base md:text-lg w-[90%] text-[#707070] hover:text-black cursor-pointer line-clamp-1">
                                        {term.heading}
                                    </span>
                                    <HiArrowTurnRightDown className="text-base md:text-lg w-[20px]" />
                                </div>
                            </Link>
                        ))}
                    </div>
                ...
                <section className={`p-5 border bg-white rounded-lg md:flex flex-col gap-y-10 w-[30%] h-fit hidden`}>
                    <h3 className="font-bold text-xl md:text-[22px] pb-3" >ON THIS PAGE</h3>
                    <div className={`flex flex-col gap-y-10`}>
                        {introduction.map((term, index) => (
                            <Link key={index} href={`#${slugify(term.heading)}`} scroll={false}>
                                <div className="flex items-center w-full justify-between gap-x-5">
                                    <span className="text-lg md:text-xl w-[90%] text-[#707070] hover:text-black cursor-pointer line-clamp-1">
                                        {term.heading}
                                    </span>
                ...
                <main className="flex flex-col w-full md:w-[70%] gap-y-4 px-0 md:px-0">
                {introduction.map((term, index) => (
                    <div key={index} id={slugify(term.heading)} className={`${index === 0 ? "pt-0" : "pt-8 md:pt-14"}`}>
                        <span className={`w-[90%] text-black font-bold text-2xl md:text-3xl`}>
                            {term.heading}
                        </span>

                        <div className="flex flex-col gap-y-5 text-base md:text-xl pt-5">
