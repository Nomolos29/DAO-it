...
const AboutDaoIt = () => {
  return (
    <section className='w-full flex justify-center'>
        <main className='flex justify-between items-end w-full max-w-screen-2xl mx-auto px-4 md:px-10 py-10 gap-x-10'>
            <div className='relative w-1/2 h-full'>
                <img src="/LandingPage/homePageAboutFrame.png" alt="About DAOit section image" />
                <img src="/LandingPage/AboutSectionHome.png" alt="" className='absolute bottom-0 left-[10%]' />
                <div className="w-[65px] h-[65px] border-[12px] border-white bg-[#D6D5FF] rounded-full absolute top-[-5px] left-[-5px] z-10" />
            </div>

            <article className='flex flex-col w-1/2 gap-y-4'>
                {AboutContent.map((content, index) => (
                    <ul key={index} className='list-disc'>
                        <li key={index} className='text-2xl font-medium text-[#5B5E65] mb-4'>
                            {content}
                        </li>
                    </ul>
                ))}
